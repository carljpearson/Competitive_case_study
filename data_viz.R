...
par <- rep(1:115,4)
task <- rep(c(1,2),460)
platform <- c(rep("Mercato",230),rep("InstaCart",230))
#create dataframe
data <- data.frame(par,task,platform) %>% mutate(task=as.factor(task))
...
#faking time data
data$time <- NA
data$time[data$task==1&data$platform=='Mercato'] <- truncnorm::rtruncnorm(n=115, a=30, b=300, mean=96, sd=25.09)
data$time[data$task==1&data$platform=='InstaCart'] <- truncnorm::rtruncnorm(n=115, a=30, b=340, mean=60, sd=60)
data$time[data$task==2&data$platform=='Mercato'] <- truncnorm::rtruncnorm(n=115, a=30, b=300, mean=170, sd=25.09)
data$time[data$task==2&data$platform=='InstaCart'] <- truncnorm::rtruncnorm(n=115, a=30, b=300, mean=175, sd=25.09)
#drop task time where not complete
data$time[data$completion==0] <- NA

#faking satisfaction data
data$satisfaction <- NA
data$satisfaction[data$task==1&data$platform=='Mercato'] <- truncnorm::rtruncnorm(n=115, a=1, b=5, mean=4.5, sd=.5)
data$satisfaction[data$task==1&data$platform=='InstaCart'] <- truncnorm::rtruncnorm(n=115, a=1, b=5, mean=4.3, sd=.5)
data$satisfaction[data$task==2&data$platform=='Mercato'] <- truncnorm::rtruncnorm(n=115, a=1, b=5, mean=4.2, sd=.8)
data$satisfaction[data$task==2&data$platform=='InstaCart'] <- truncnorm::rtruncnorm(n=115, a=1, b=5, mean=3.6, sd=.8)
#convert to .5 rounding
...
#completion
mod.comp <- glmer(completion ~ task * platform + (1|par), binomial,data=data)
tab_model(mod.comp)
plot_model(mod.comp)
...
#time
mod.time <- lmer(time ~ task * platform + (1|par), data=data)
tab_model(mod.time)
plot_model(mod.time)
anova(mod.time)
...
#time specifications
data$tspec[data$task==1] <- 70 #time spec for task 1 in seconds
#based on derivation from data, 80th percentile from sat above 4 could be used 
quantile(data$time[data$sat >= 4 & data$task==1], probs = seq(0, 1, by= 0.1))

data$tspec[data$task==2] <- 150 #time spec for task 2 in seconds
#or based on derivation from data, 80th percentile from sat above 4
quantile(data$time[data$sat >= 4 & data$task==2], probs = seq(0, 1, by= 0.1))
...
df <- data %>% rename(group=platform,sat=satisfaction,comp=completion)
...
df_summarised$Group <- factor(df_summarised$Group, levels = c("Mercato","InstaCart"))  #reorder factor groups

df_summarised$group <- factor(df_summarised$group, levels = c("Mercato","InstaCart"))  #reorder factor groups

#add better labels for task var, mainly used for plotting but function as factor variable
df_summarised$task_named[df_summarised$task==1] <- "Task 1: Signup"
df_summarised$task_named[df_summarised$task==2] <- "Task 2: Find info"
...
df_task$task_named[df_task$task==1] <- "Task 1: Signup"
df_task$task_named[df_task$task==2]  <- "Task 2: Find info"
...
df_summarised %>%
  ...
  mutate(Group=factor(Group, levels = c("3.5","3.11")))-> df_SUM
...
for(i in unique(df_summarised$task)){
  ...
  df_summarised %>%
    ...
    mutate(hjust_value=ifelse(group=="Mercado",1.5,-.5)) %>%
    ggplot(aes(x=Measure, y=point_est.nps,fill=group)) + 
